...
def main():
    ...
    data_transforms = transforms.Compose([
        transforms.Resize((105, 105)),                          # Tamanho padrão
        transforms.RandomHorizontalFlip(),                      # Reflexão horizontal
        transforms.RandomRotation(20),                          # Rotação entre -20° e +20°
        transforms.ColorJitter(brightness=0.3, contrast=0.3),   # Variações de luz
    ...
    test_dataset = OmniglotTest(raw_test_dataset, transform=test_transforms, times=cfg.times, way=cfg.way)
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.learning_rate, weight_decay=1e-4)  # L2 regularização
